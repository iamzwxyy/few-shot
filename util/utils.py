...
class IEDatasetFake(Dataset):
    ...
    def __init__(self, file_path, tokenizer, max_seq_len) -> None:
        super().__init__()
        self.file_path = file_path
        self.dataset = list(readers_fake(file_path))
        self.tokenizer = tokenizer
        self.max_seq_len = max_seq_len
    ...
    def __getitem__(self, index):
        return convert_example_fake(self.dataset[index], tokenizer=self.tokenizer, max_seq_len=self.max_seq_len)


def readers_fake(data_path, max_seq_len=512):
...
def convert_example_fake(example, tokenizer, max_seq_len):
    """
    example: {
        title
        prompt
...
def add_negative_example(examples, texts, prompts, label_set, negative_ratio):
    """
    实体--[{"content": text, "result_list": [result], "prompt": 中标金额},{"content": text, "result_list": [result], "prompt": 中标金额}]
    长文本
    实体名称 --中标金额
    【中标金额，采购单位】
    Args:
    ...
    with tqdm(total=len(prompts)) as pbar:
        for i, prompt in enumerate(prompts):
            negative_sample = []
            redundants_list = list(set(label_set) ^ set(prompt))  # 两个集合的并集
